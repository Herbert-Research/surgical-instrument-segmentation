...
def analyze_mask_statistics(generated_dir: Path) -> None:
    ...
    print(f"  Frames with instruments: {frames_with_instruments}/{len(mask_files)} ({frames_with_instruments/len(mask_files)*100:.1f}%)")
    ...
    ax1.axvline(np.mean(instrument_coverages), color='red', linestyle='--', 
                linewidth=2, label=f'Mean: {np.mean(instrument_coverages):.2f}%')
    ...
    # Frame-by-frame plot
    ax2.plot(range(len(instrument_coverages)), instrument_coverages, 
             'o-', linewidth=2, markersize=6, color='steelblue')
    ax2.axhline(np.mean(instrument_coverages), color='red', linestyle='--', 
                linewidth=2, label=f'Mean: {np.mean(instrument_coverages):.2f}%')
