...
class InstrumentSegmentationModel(torch.nn.Module):
...
def generate_visualization(
    ...
    selected_frames = [frames_with_instruments[i * step][0] for i in range(min(num_samples, len(frames_with_instruments)))]
    ...
    with torch.no_grad():
        for idx, frame_stem in enumerate(selected_frames):
            ...
            true_mask = np.array(true_mask_pil.resize((256, 256), Image.Resampling.NEAREST), dtype=np.uint8)
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("impressive_segmentation_results.png"),
