"""
Generate curated best-case visualizations for demo purposes.

This script intentionally selects frames with high instrument presence. Do not
present these outputs as representative performance.
"""
...
DEFAULT_OUTPUT_PATH: Path = Path("outputs/figures/demo_best_cases.png")
DEFAULT_MIN_INSTRUMENT_PIXELS: int = 1000
DEFAULT_RANDOM_SEED: int = 1337


def count_instrument_pixels(mask_array: np.ndarray) -> int:
    """Count instrument pixels in a grayscale mask.

    Args:
        mask_array: Grayscale mask array with class IDs.

    Returns:
        Count of pixels that correspond to instrument classes.
    """
    return int(np.sum((mask_array == 1) | (mask_array == 31) | (mask_array == 32)))


def find_frames_with_instruments(
    mask_dir: Path, min_instrument_pixels: int
) -> list[tuple[str, int]]:
    """Find frames that have significant instrument presence.

    Args:
        mask_dir: Directory containing mask images.
        min_instrument_pixels: Minimum instrument pixel count to qualify.

    Returns:
        List of (frame_stem, instrument_pixel_count) sorted by pixel count.
    """
    mask_files = sorted(mask_dir.glob("*.png"))
    frames_with_instruments: list[tuple[str, int]] = []

    print(f"Scanning {len(mask_files)} masks for frames with instruments...")

    for mask_path in mask_files:
        mask = np.array(Image.open(mask_path).convert("L"))
        instrument_pixel_count = count_instrument_pixels(mask)

        if instrument_pixel_count >= min_instrument_pixels:
            frames_with_instruments.append((mask_path.stem, instrument_pixel_count))

    # Sort by number of instrument pixels (descending) to get most interesting frames
    frames_with_instruments.sort(key=lambda entry: entry[1], reverse=True)

    print(f"Found {len(frames_with_instruments)} frames with instruments")
    return frames_with_instruments


def select_best_case_frames(
    frames_with_instruments: list[tuple[str, int]], num_samples: int
) -> list[str]:
    """Select curated best-case frames from a ranked list.

    Args:
        frames_with_instruments: Ranked list of frames by instrument pixels.
    ...
    return [
        frames_with_instruments[index * step][0]
        for index in range(min(num_samples, len(frames_with_instruments)))
    ]
...
def generate_visualization(
    ...
    output_path: Path,
    num_samples: int,
    min_instrument_pixels: int,
    random_sample: bool,
    seed: int,
) -> None:
    """Generate visualization with curated or random frames.
    ...
    Args:
        ...
        min_instrument_pixels: Minimum instrument pixels for curated selection.
        random_sample: Whether to use random sampling instead of curated selection.
        seed: Random seed used when random_sample is True.
    """
    _ = pred_dir
    ...
    if random_sample:
        ...
        frames_with_instruments = find_frames_with_instruments(
            mask_dir, min_instrument_pixels=min_instrument_pixels
        )
        selected_frames = select_best_case_frames(frames_with_instruments, num_samples)
...
def main() -> None:
    """CLI entry point for curated demo visualizations."""
    parser = argparse.ArgumentParser(
        description="Generate curated best-case segmentation visualizations (demo only)"
    )
    parser.add_argument(
        "--frame-dir",
        type=Path,
    ...
    parser.add_argument(
        "--min-instrument-pixels",
        type=int,
        default=DEFAULT_MIN_INSTRUMENT_PIXELS,
        help="Minimum instrument pixels to qualify for curated selection",
    )
    parser.add_argument(
        "--random-sample",
        action="store_true",
        help="Use random sampling instead of curated best-case selection",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=DEFAULT_RANDOM_SEED,
        help="Random seed recorded when --random-sample is used",
    )

    args = parser.parse_args()

    print("=" * 70)
    print("DEMO BEST-CASE SEGMENTATION VISUALIZATION")
    print("=" * 70)
    ...
    generate_visualization(
        ...
        min_instrument_pixels=args.min_instrument_pixels,
